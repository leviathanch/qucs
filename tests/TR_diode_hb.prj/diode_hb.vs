(* qucs_ViewX1=0, qucs_ViewY1=0, qucs_ViewX2=860, qucs_ViewY2=660, qucs_Scale=1, qucs_tmpViewX1=0, qucs_tmpViewY1=0, qucs_GridX=10, qucs_GridY=10, qucs_GridOn=1, qucs_DataSet="diode_hb.dat", qucs_DataDisplay="diode_hb.dpl", qucs_SimOpenDpl=0, qucs_Script="diode_hb.m", qucs_SimRunScript=0, qucs_showFrame=0, qucs_FrameText0="Title", qucs_FrameText1="Drawn By:", qucs_FrameText2="Date:", qucs_FrameText3="Revision:" *) module diode_hb();
    ...
    (* qucs_mirrored=0, qucs_rotated=0,  *) \.HB  #(.f(1 GHz),.n(8),.iabstol(1 pA),.vabstol(1 uV),.reltol(0.001),.MaxIter(150)) HB1 (  );
    (* qucs_mirrored=0, qucs_rotated=0,  *) Eqn #(.Spectrum(Time2Freq(Node3.Vt, time)),.Export(yes)) Eqn1 (  );
    (* qucs_mirrored=0, qucs_rotated=0,  *) \.TR  #(.Type(lin),.Start(0),.Stop(4 ns),.Points(256),.IntegrationMethod(Trapezoidal),.Order(2),.InitialStep(1 ns),.MinStep(1e-16),.MaxIter(150),.reltol(0.001),.abstol(1 pA),.vntol(1 uV),.Temp(26.85),.LTEreltol(1e-3),.LTEabstol(1e-6),.LTEfactor(1),.Solver(CroutLU),.relaxTSR(no),.initialDC(yes),.MaxStep(0)) TR1 (  );
