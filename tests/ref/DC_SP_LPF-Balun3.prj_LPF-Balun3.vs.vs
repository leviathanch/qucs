(* qucs_ViewX1=0, qucs_ViewY1=0, qucs_ViewX2=800, qucs_ViewY2=800, qucs_Scale=1, qucs_tmpViewX1=-200, qucs_tmpViewY1=-200, qucs_GridX=10, qucs_GridY=10, qucs_GridOn=1, qucs_DataSet="LPF-Balun3.dat", qucs_DataDisplay="LPF-Balun3.dpl", qucs_SimOpenDpl=1, qucs_Script="LPF-Balun3.m", qucs_SimRunScript=0, qucs_showFrame=0, qucs_FrameText0="Title", qucs_FrameText1="Drawn By:", qucs_FrameText2="Date:", qucs_FrameText3="Revision:" *) module LPF-Balun3();
    wire n_40_140;
    wire n_40_190;
    wire n_140_190;
    wire n_260_150;
    wire n_300_180;
    wire n_300_210;
    wire n_360_80;
    wire n_360_250;
    wire n_470_190;
    wire n_510_80;
    wire n_510_250;
    wire n_610_190;
    wire n_650_80;
    wire n_650_250;
    wire n_820_150;
    wire n_890_220;
    wire n_930_270;
    wire n_940_310;
    wire n_960_130;
    wire n_960_200;
    (* qucs_mirrored=0, qucs_rotated=1, S0_x1=40, S0_y1=110, S0_x2=40, S0_y2=170 *) Pac #(.Num(1),.Z(100 Ohm),.P(0 dBm),.f(1 GHz),.Temp(26.85)) P1 ( n_40_110, n_40_170 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=480, S0_y1=80, S0_x2=540, S0_y2=80 *) L #(.L(1.2uH),.I()) L1 ( n_480_80, n_540_80 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=620, S0_y1=80, S0_x2=680, S0_y2=80 *) L #(.L(470nH),.I()) L2 ( n_620_80, n_680_80 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=440, S0_y1=190, S0_x2=500, S0_y2=190 *) C #(.C(270pF),.V(),.Symbol(neutral)) C1 ( n_440_190, n_500_190 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=580, S0_y1=190, S0_x2=640, S0_y2=190 *) C #(.C(270pF),.V(),.Symbol(neutral)) C2 ( n_580_190, n_640_190 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=330, S0_y1=80, S0_x2=390, S0_y2=80 *) L #(.L(470nH),.I()) L3 ( n_330_80, n_390_80 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=40, S0_y1=190 *) GND #() \\*  ( n_40_190 );
    (* qucs_mirrored=0, qucs_rotated=1, S0_x1=960, S0_y1=100, S0_x2=960, S0_y2=160 *) Pac #(.Num(2),.Z(100 Ohm),.P(0 dBm),.f(1 GHz),.Temp(26.85)) P2 ( n_960_100, n_960_160 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=960, S0_y1=200 *) GND #() \\*  ( n_960_200 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=330, S0_y1=250, S0_x2=390, S0_y2=250 *) L #(.L(470nH),.I()) L4 ( n_330_250, n_390_250 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=480, S0_y1=250, S0_x2=540, S0_y2=250 *) L #(.L(1.2uH),.I()) L5 ( n_480_250, n_540_250 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=620, S0_y1=250, S0_x2=680, S0_y2=250 *) L #(.L(470nH),.I()) L6 ( n_620_250, n_680_250 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=230, S0_y1=80, S0_x2=290, S0_y2=120, S0_x3=290, S0_y3=180, S0_x4=230, S0_y4=220, S0_x5=230, S0_y5=160, S0_x6=230, S0_y6=140 *) sTr #(.T1(0.5),.T2(0.5)) Tr5 ( n_230_80, n_290_120, n_290_180, n_230_220, n_230_160, n_230_140 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=790, S0_y1=80, S0_x2=850, S0_y2=120, S0_x3=850, S0_y3=180, S0_x4=790, S0_y4=220, S0_x5=790, S0_y5=160, S0_x6=790, S0_y6=140 *) sTr #(.T1(0.5),.T2(0.5)) Tr6 ( n_790_80, n_850_120, n_850_180, n_790_220, n_790_160, n_790_140 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=890, S0_y1=220 *) GND #() \\*  ( n_890_220 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=140, S0_y1=190 *) GND #() \\*  ( n_140_190 );
    (* qucs_mirrored=0, qucs_rotated=1, S0_x1=300, S0_y1=150, S0_x2=300, S0_y2=210 *) Vdc #(.U(1V)) V1 ( n_300_150, n_300_210 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=300, S0_y1=210 *) GND #() \\*  ( n_300_210 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=920, S0_y1=290, S0_x2=940, S0_y2=290 *) VProbe #() Pr1 ( n_920_290, n_940_290 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=940, S0_y1=310 *) GND #() \\*  ( n_940_310 );
    (* qucs_mirrored=0, qucs_rotated=1, S0_x1=40, S0_y1=110, S0_x2=40, S0_y2=170 *) Pac #(.Num(1),.Z(100 Ohm),.P(0 dBm),.f(1 GHz),.Temp(26.85)) P1 ( n_40_110, n_40_170 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=480, S0_y1=80, S0_x2=540, S0_y2=80 *) L #(.L(1.2uH),.I()) L1 ( n_480_80, n_540_80 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=620, S0_y1=80, S0_x2=680, S0_y2=80 *) L #(.L(470nH),.I()) L2 ( n_620_80, n_680_80 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=440, S0_y1=190, S0_x2=500, S0_y2=190 *) C #(.C(270pF),.V(),.Symbol(neutral)) C1 ( n_440_190, n_500_190 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=580, S0_y1=190, S0_x2=640, S0_y2=190 *) C #(.C(270pF),.V(),.Symbol(neutral)) C2 ( n_580_190, n_640_190 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=330, S0_y1=80, S0_x2=390, S0_y2=80 *) L #(.L(470nH),.I()) L3 ( n_330_80, n_390_80 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=40, S0_y1=190 *) GND #() \\*  ( n_40_190 );
    (* qucs_mirrored=0, qucs_rotated=1, S0_x1=960, S0_y1=100, S0_x2=960, S0_y2=160 *) Pac #(.Num(2),.Z(100 Ohm),.P(0 dBm),.f(1 GHz),.Temp(26.85)) P2 ( n_960_100, n_960_160 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=960, S0_y1=200 *) GND #() \\*  ( n_960_200 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=330, S0_y1=250, S0_x2=390, S0_y2=250 *) L #(.L(470nH),.I()) L4 ( n_330_250, n_390_250 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=480, S0_y1=250, S0_x2=540, S0_y2=250 *) L #(.L(1.2uH),.I()) L5 ( n_480_250, n_540_250 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=620, S0_y1=250, S0_x2=680, S0_y2=250 *) L #(.L(470nH),.I()) L6 ( n_620_250, n_680_250 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=230, S0_y1=80, S0_x2=290, S0_y2=120, S0_x3=290, S0_y3=180, S0_x4=230, S0_y4=220, S0_x5=230, S0_y5=160, S0_x6=230, S0_y6=140 *) sTr #(.T1(0.5),.T2(0.5)) Tr5 ( n_230_80, n_290_120, n_290_180, n_230_220, n_230_160, n_230_140 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=790, S0_y1=80, S0_x2=850, S0_y2=120, S0_x3=850, S0_y3=180, S0_x4=790, S0_y4=220, S0_x5=790, S0_y5=160, S0_x6=790, S0_y6=140 *) sTr #(.T1(0.5),.T2(0.5)) Tr6 ( n_790_80, n_850_120, n_850_180, n_790_220, n_790_160, n_790_140 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=890, S0_y1=220 *) GND #() \\*  ( n_890_220 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=140, S0_y1=190 *) GND #() \\*  ( n_140_190 );
    (* qucs_mirrored=0, qucs_rotated=1, S0_x1=300, S0_y1=150, S0_x2=300, S0_y2=210 *) Vdc #(.U(1V)) V1 ( n_300_150, n_300_210 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=300, S0_y1=210 *) GND #() \\*  ( n_300_210 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=920, S0_y1=290, S0_x2=940, S0_y2=290 *) VProbe #() Pr1 ( n_920_290, n_940_290 );
    (* qucs_mirrored=0, qucs_rotated=0, S0_x1=940, S0_y1=310 *) GND #() \\*  ( n_940_310 );
endmodule
