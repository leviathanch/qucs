    ...
    Schematic const* s)
{
  ...
  if(c) {
    ...
    // BUG. Wire is not a Component.
    stream << ((Wire*)w)->ports(0)->label();
    stream << ", ";
    stream << ((Wire*)w)->ports(1)->label();
  } else{
    unreachable();
  }
  stream << " );\n";
...
int Schematic::saveVerilogDocument(QFile *file)
{
  ...
  for (auto it = DocComps.begin(); it != DocComps.end(); ++it) {
    if(it->obsolete_model_hack() == "Port") {
      ...
      for(int i=0; i<it->net_nodes(); i++) {
        if((std::find(wireList.begin(), wireList.end(), it->port(i).getConnection()->label())== wireList.end())) {
          wireList.push_back(it->port(i).getConnection()->label());
        }
      }
    }
  }
  ...
  for (auto it = DocWires.begin(); it != DocWires.end(); ++it) {
    for(int i=0; i<2; i++) {
      if((std::find(wireList.begin(), wireList.end(), it->ports(i)->label())==wireList.end())) {
        wireList.push_back(it->ports(i)->label());
      }
    }
  }
